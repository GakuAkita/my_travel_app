// GENERATED CODE - DO NOT MODIFY BY HAND
// coverage:ignore-file
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'balance_info.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

// dart format off
T _$identity<T>(T value) => value;

/// @nodoc
mixin _$BalanceInfo {

 String get uid; double get netTotal; double get reimbursedSum; double get paidSum;
/// Create a copy of BalanceInfo
/// with the given fields replaced by the non-null parameter values.
@JsonKey(includeFromJson: false, includeToJson: false)
@pragma('vm:prefer-inline')
$BalanceInfoCopyWith<BalanceInfo> get copyWith => _$BalanceInfoCopyWithImpl<BalanceInfo>(this as BalanceInfo, _$identity);

  /// Serializes this BalanceInfo to a JSON map.
  Map<String, dynamic> toJson();


@override
bool operator ==(Object other) {
  return identical(this, other) || (other.runtimeType == runtimeType&&other is BalanceInfo&&(identical(other.uid, uid) || other.uid == uid)&&(identical(other.netTotal, netTotal) || other.netTotal == netTotal)&&(identical(other.reimbursedSum, reimbursedSum) || other.reimbursedSum == reimbursedSum)&&(identical(other.paidSum, paidSum) || other.paidSum == paidSum));
}

@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hash(runtimeType,uid,netTotal,reimbursedSum,paidSum);

@override
String toString() {
  return 'BalanceInfo(uid: $uid, netTotal: $netTotal, reimbursedSum: $reimbursedSum, paidSum: $paidSum)';
}


}

/// @nodoc
abstract mixin class $BalanceInfoCopyWith<$Res>  {
  factory $BalanceInfoCopyWith(BalanceInfo value, $Res Function(BalanceInfo) _then) = _$BalanceInfoCopyWithImpl;
@useResult
$Res call({
 String uid, double netTotal, double reimbursedSum, double paidSum
});




}
/// @nodoc
class _$BalanceInfoCopyWithImpl<$Res>
    implements $BalanceInfoCopyWith<$Res> {
  _$BalanceInfoCopyWithImpl(this._self, this._then);

  final BalanceInfo _self;
  final $Res Function(BalanceInfo) _then;

/// Create a copy of BalanceInfo
/// with the given fields replaced by the non-null parameter values.
@pragma('vm:prefer-inline') @override $Res call({Object? uid = null,Object? netTotal = null,Object? reimbursedSum = null,Object? paidSum = null,}) {
  return _then(_self.copyWith(
uid: null == uid ? _self.uid : uid // ignore: cast_nullable_to_non_nullable
as String,netTotal: null == netTotal ? _self.netTotal : netTotal // ignore: cast_nullable_to_non_nullable
as double,reimbursedSum: null == reimbursedSum ? _self.reimbursedSum : reimbursedSum // ignore: cast_nullable_to_non_nullable
as double,paidSum: null == paidSum ? _self.paidSum : paidSum // ignore: cast_nullable_to_non_nullable
as double,
  ));
}

}


/// Adds pattern-matching-related methods to [BalanceInfo].
extension BalanceInfoPatterns on BalanceInfo {
/// A variant of `map` that fallback to returning `orElse`.
///
/// It is equivalent to doing:
/// ```dart
/// switch (sealedClass) {
///   case final Subclass value:
///     return ...;
///   case _:
///     return orElse();
/// }
/// ```

@optionalTypeArgs TResult maybeMap<TResult extends Object?>(TResult Function( _BalanceInfo value)?  $default,{required TResult orElse(),}){
final _that = this;
switch (_that) {
case _BalanceInfo() when $default != null:
return $default(_that);case _:
  return orElse();

}
}
/// A `switch`-like method, using callbacks.
///
/// Callbacks receives the raw object, upcasted.
/// It is equivalent to doing:
/// ```dart
/// switch (sealedClass) {
///   case final Subclass value:
///     return ...;
///   case final Subclass2 value:
///     return ...;
/// }
/// ```

@optionalTypeArgs TResult map<TResult extends Object?>(TResult Function( _BalanceInfo value)  $default,){
final _that = this;
switch (_that) {
case _BalanceInfo():
return $default(_that);case _:
  throw StateError('Unexpected subclass');

}
}
/// A variant of `map` that fallback to returning `null`.
///
/// It is equivalent to doing:
/// ```dart
/// switch (sealedClass) {
///   case final Subclass value:
///     return ...;
///   case _:
///     return null;
/// }
/// ```

@optionalTypeArgs TResult? mapOrNull<TResult extends Object?>(TResult? Function( _BalanceInfo value)?  $default,){
final _that = this;
switch (_that) {
case _BalanceInfo() when $default != null:
return $default(_that);case _:
  return null;

}
}
/// A variant of `when` that fallback to an `orElse` callback.
///
/// It is equivalent to doing:
/// ```dart
/// switch (sealedClass) {
///   case Subclass(:final field):
///     return ...;
///   case _:
///     return orElse();
/// }
/// ```

@optionalTypeArgs TResult maybeWhen<TResult extends Object?>(TResult Function( String uid,  double netTotal,  double reimbursedSum,  double paidSum)?  $default,{required TResult orElse(),}) {final _that = this;
switch (_that) {
case _BalanceInfo() when $default != null:
return $default(_that.uid,_that.netTotal,_that.reimbursedSum,_that.paidSum);case _:
  return orElse();

}
}
/// A `switch`-like method, using callbacks.
///
/// As opposed to `map`, this offers destructuring.
/// It is equivalent to doing:
/// ```dart
/// switch (sealedClass) {
///   case Subclass(:final field):
///     return ...;
///   case Subclass2(:final field2):
///     return ...;
/// }
/// ```

@optionalTypeArgs TResult when<TResult extends Object?>(TResult Function( String uid,  double netTotal,  double reimbursedSum,  double paidSum)  $default,) {final _that = this;
switch (_that) {
case _BalanceInfo():
return $default(_that.uid,_that.netTotal,_that.reimbursedSum,_that.paidSum);case _:
  throw StateError('Unexpected subclass');

}
}
/// A variant of `when` that fallback to returning `null`
///
/// It is equivalent to doing:
/// ```dart
/// switch (sealedClass) {
///   case Subclass(:final field):
///     return ...;
///   case _:
///     return null;
/// }
/// ```

@optionalTypeArgs TResult? whenOrNull<TResult extends Object?>(TResult? Function( String uid,  double netTotal,  double reimbursedSum,  double paidSum)?  $default,) {final _that = this;
switch (_that) {
case _BalanceInfo() when $default != null:
return $default(_that.uid,_that.netTotal,_that.reimbursedSum,_that.paidSum);case _:
  return null;

}
}

}

/// @nodoc
@JsonSerializable()

class _BalanceInfo implements BalanceInfo {
  const _BalanceInfo({required this.uid, required this.netTotal, required this.reimbursedSum, required this.paidSum});
  factory _BalanceInfo.fromJson(Map<String, dynamic> json) => _$BalanceInfoFromJson(json);

@override final  String uid;
@override final  double netTotal;
@override final  double reimbursedSum;
@override final  double paidSum;

/// Create a copy of BalanceInfo
/// with the given fields replaced by the non-null parameter values.
@override @JsonKey(includeFromJson: false, includeToJson: false)
@pragma('vm:prefer-inline')
_$BalanceInfoCopyWith<_BalanceInfo> get copyWith => __$BalanceInfoCopyWithImpl<_BalanceInfo>(this, _$identity);

@override
Map<String, dynamic> toJson() {
  return _$BalanceInfoToJson(this, );
}

@override
bool operator ==(Object other) {
  return identical(this, other) || (other.runtimeType == runtimeType&&other is _BalanceInfo&&(identical(other.uid, uid) || other.uid == uid)&&(identical(other.netTotal, netTotal) || other.netTotal == netTotal)&&(identical(other.reimbursedSum, reimbursedSum) || other.reimbursedSum == reimbursedSum)&&(identical(other.paidSum, paidSum) || other.paidSum == paidSum));
}

@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hash(runtimeType,uid,netTotal,reimbursedSum,paidSum);

@override
String toString() {
  return 'BalanceInfo(uid: $uid, netTotal: $netTotal, reimbursedSum: $reimbursedSum, paidSum: $paidSum)';
}


}

/// @nodoc
abstract mixin class _$BalanceInfoCopyWith<$Res> implements $BalanceInfoCopyWith<$Res> {
  factory _$BalanceInfoCopyWith(_BalanceInfo value, $Res Function(_BalanceInfo) _then) = __$BalanceInfoCopyWithImpl;
@override @useResult
$Res call({
 String uid, double netTotal, double reimbursedSum, double paidSum
});




}
/// @nodoc
class __$BalanceInfoCopyWithImpl<$Res>
    implements _$BalanceInfoCopyWith<$Res> {
  __$BalanceInfoCopyWithImpl(this._self, this._then);

  final _BalanceInfo _self;
  final $Res Function(_BalanceInfo) _then;

/// Create a copy of BalanceInfo
/// with the given fields replaced by the non-null parameter values.
@override @pragma('vm:prefer-inline') $Res call({Object? uid = null,Object? netTotal = null,Object? reimbursedSum = null,Object? paidSum = null,}) {
  return _then(_BalanceInfo(
uid: null == uid ? _self.uid : uid // ignore: cast_nullable_to_non_nullable
as String,netTotal: null == netTotal ? _self.netTotal : netTotal // ignore: cast_nullable_to_non_nullable
as double,reimbursedSum: null == reimbursedSum ? _self.reimbursedSum : reimbursedSum // ignore: cast_nullable_to_non_nullable
as double,paidSum: null == paidSum ? _self.paidSum : paidSum // ignore: cast_nullable_to_non_nullable
as double,
  ));
}


}

// dart format on
