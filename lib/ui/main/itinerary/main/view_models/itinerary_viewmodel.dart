...
class ItineraryViewModel extends ChangeNotifier {
  ...
  Future<ResultInfo> switchEditModePreCheckWithNotify() async {
    _isEditLoading = true;
    notifyListeners();
    final result = await switchToEditModePrecheck();
  ...
  Future<ResultInfo> switchToEditModePrecheck() async {
  ...
  Future<ResultInfo> saveItinerarySections() async {
    ...
    try {
      ...
      await _itineraryRepository.saveItinerarySections(
        groupId: groupId,
        travelId: travelId,
        sections: _editingItinerarySections,
      );
      return ResultInfo.success();
    } catch (e) {
      return ResultInfo.failed(error: ErrorInfo(errorMessage: e.toString()));
