...
class _EstimatedExpenseScreenState extends State<EstimatedExpenseScreen> {
  ...
  ResultInfo _createEstimatedListFromItinerary() {
    ...
    final tables =
        itineraryStore.getData().where((s) => s.type == ItinerarySectionType.defaultTable).toList();
  ...
  Future<ResultInfo> _loadEstimatedExpenseFromManual() async {
    ...
    if (!checkIsShownTravelInput(shownTravel).isSuccess) {
      return ResultInfo.failed(
        error: ErrorInfo(
          ...
          errorMessage: "Invalid travel basic data. This is the bug. Let the developer know.",
        ),
      );
    }
    ...
    final estRet = await FirebaseDatabaseService.getSingleTravelEstimatedExpensesData(groupId, travelId);
    if (!estRet.isSuccess) {
      return ResultInfo.failed(error: ErrorInfo(errorMessage: "${estRet.error?.errorMessage}"));
    }

    final lastUpdateRet = await FirebaseDatabaseService.getSingleTravelEstimatedUpdateDate(groupId, travelId);
    if (!lastUpdateRet.isSuccess) {
      return ResultInfo.failed(error: ErrorInfo(errorMessage: "${lastUpdateRet.error?.errorMessage}"));
    }
  ...
  void _sumEstimatedExpenseList() {
    ...
    setState(() {
      estimatedExpense = estimatedExpenseFromItinerary + estimatedExpenseFromManual;
    });
  }
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              if (estimatedListFromManual != null)
                Column(
                  children: List.generate(estimatedListFromManual!.length, (index) {
                    return EstimatedExpenseRow(
                      ...
                      onDelete: (estimated) {
                        print("Deleted ${estimated.expenseItem} ${estimated.amount}");
                        setState(() {
                ...
                onPressed: () async {
                  if (estimatedListFromManual == null) {
                    ScaffoldMessenger.of(
                      context,
                    ).showSnackBar(const SnackBar(content: Text("estimatedListFromManualがnullです")));
                    return;
                  }
                  ...
                  for (final est in estimatedListFromManual!) {
                    if (est.expenseItem == "") {
                      ...
                      ScaffoldMessenger.of(
                        context,
                      ).showSnackBar(const SnackBar(content: Text("使うものが空になっています")));
                      return;
                    }
                    ...
                    if (est.amount < 0) {
                      ...
                      ScaffoldMessenger.of(
                        context,
                      ).showSnackBar(SnackBar(content: Text("総額が負になっています。${est.amount}")));
                      return;
                    }
                    ...
                    if (est.reimbursedByCnt < 1 || est.reimbursedByCnt > 99) {
                      ...
                      ScaffoldMessenger.of(
                        context,
                      ).showSnackBar(SnackBar(content: Text("人数が正しくありません。${est.reimbursedByCnt}")));
                      return;
                    }
                  }
                  ...
                  if (!checkIsShownTravelInput(shownTravel).isSuccess) {
                    ...
                    ScaffoldMessenger.of(
                      context,
                    ).showSnackBar(SnackBar(content: Text("旅行が適切に選択されていません。開発者に連絡してください")));
                    return;
                  }
                  ...
                  // );
                  print("保存に成功しました");
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("保存に成功しました")));
                },
              ),
            ],
          ),
...
class EstimatedExpenseInfo {
  ...
  EstimatedExpenseInfo copyWith({String? id, String? expenseItem, double? amount, int? reimbursedByCnt}) {
  ...
  Map<String, dynamic> toMap() {
    return {'id': id, 'expenseItem': expenseItem, 'amount': amount, 'reimbursedByCnt': reimbursedByCnt};
  }
}
